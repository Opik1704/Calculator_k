...
class Calculator:
    ...
    def get_current_token(self) -> Optional[str]:
        """ Функция возвращает текущий токен или None если достигнут конец"""
    ...
    def consume_token(self, expected_token: Optional[str] = None) -> str:
        """ Берет текущий токен и перемещается к следующему
        Получает ожидаемый токен (если None, проверка не выполняется)
        Возращает токен
        Ошибка Если достигнут конец или токен не соответствует ожидаемому
        """
        if self._current_position >= len(self._tokens):
    ...
    def parse_number(self, token: str) -> Union[int, float]:
        ...
        Получает токен для преобразования
        Возращает Числовое значение токена
        Ошибка если токен не может быть преобразован в число
        """
    ...
    def handle_primary_expression(self) -> Union[int, float]:
        ...
        Возвращает результат вычисления первичного выражения
        Ошибка при синтаксических ошибках в выражении
        """
    ...
    def handle_function_call(self, function_name: str) -> Union[int, float]:
        """
        Обработчик вызовов функции
        Получает имя вызываемой функции
        Возвращает результат вызова функции
        Ошбика если функция неизвестна или ошибка в аргументах
        """
    ...
    def handle_variable_reference(self, variable_name: str) -> Union[int, float]:
        """
        Обработчик ссылки на переменную
        Получает имя переменной
        Возвращает значение переменной
        Ошибка если переменная не объявлена
        """
    ...
    def handle_assignment(self) -> Union[int, float]:
        """
        Обработчик присваивания переменных let x
        Возвращает значение присваивания
        Ошибка при ошибках в синтаксисе присваивания
        """
        token = self.get_current_token()
    ...
    def calculate(self, expression: str) -> Union[int, float]:
        ...
        Получает expression: Математическое выражение для вычисления
        Возвращает результат выражения
        Ошибка  если выражение содержит ошибки
        """
    ...
    def get_variable_value(self, variable_name: str) -> Optional[Union[int, float]]:
        """
        Возвращает значение переменной.
        Ошибка значение переменной или None если переменная не существует
        """
